...
`.env.example` is the one public-safe environment template. It contains only deployment-varying browser inputs (`API_BASE_URL`, `APP_ENVIRONMENT`, `DEBUG_MODE`, and `LOG_LEVEL`) plus `WEBPACK_*` inputs consumed directly by Webpack Dev Server. Authentication paths, session durations, app identity, and locale defaults are code-level constants. Production values must be injected into the build environment before Webpack creates the static bundle; no production example file is maintained.
...
- `.env.example` — single public-safe local development and deployment input contract.
