...
class ReportGenerator {
  ...
  buildExecutiveSummaryRows(summaryData = {}, report = {}) {
    ...
    return [
      [
        "Attendance Rate",
        this.formatPeriodSummaryPercent(periodSummary.attendance_rate),
      ],
      [
        "Late / Alpha Risk",
  ...
  buildDisciplineLabelLookup(detailRows = []) {
    ...
    detailRows.forEach((row) => {
      ...
      if (
        !this.hasAvailableValue(userId) ||
        !this.hasAvailableValue(disciplineLabel)
      ) {
        return;
      }
  ...
  resolveDisciplineLabelForInsight(
    item = {},
    disciplineLabelLookup = new Map(),
  ) {
  ...
  buildPdfHeaderMetadataRows(period = "all", generatedAt = new Date()) {
    return [
      ...
      [
        "Data source",
        "/summary/reports payload for selected report/export period",
      ],
    ];
  }
    ...
    return [
      {
        ...
        state: this.hasAvailableValue(attendanceRate)
          ? "ready"
          : "backendRequired",
        accent: [139, 92, 246],
      },
