...
- The dashboard now loads four explicit backend sources with separate responsibilities: selected-period `/summary` data for report/export flows, `/summary/dashboard-analytics` for cockpit analytics panels, `/attendance/today-locations` for the live-map hero, and `/analysis/fuzzy-ahp` for decision-support output.
- `src/js/services/reportService.js` keeps these requests transport-oriented, while `src/js/services/dashboardCockpitService.js` unwraps source-specific payloads and decides whether each cockpit panel may become `ready`, `needsData`, `backendRequired`, or `error`.
...
- Fuzzy AHP Decision Center may become `ready` only from explicit `analysis.fuzzy-ahp` rankings and optional consistency ratio values. Geofence Evidence Context remains a backend-required placeholder and must not imply completed analytics authority.
...
We will keep `reportService` transport-oriented for `/summary`, `/summary/dashboard-analytics`, `/attendance/today-locations`, and `/analysis/fuzzy-ahp`. Cockpit-specific envelope unwrapping and no-fallback normalization belong in `dashboardCockpitService`, not in the shared request layer.
...
The live-map hero, analytics trend/mode panels, and Fuzzy AHP panel now rely on separate explicit backend contracts rather than a single selected-period report payload. That separation matters: if Web FE reuses report rows for the live map, treats analytics snapshot context as current-location truth, or derives attendance trend from summary totals, the UI would manufacture analytical truth that the backend never actually exposed.
...
- `src/js/features/dashboard/dashboard.js` — dashboard loads selected-period report, dashboard analytics, today-locations, and fuzzy-ahp sources while keeping cockpit state conservative when any explicit feed fails or is incomplete.
...
- `src/js/services/dashboardCockpitService.js` — Fuzzy AHP now has an explicit backend-backed ready path, while Geofence Evidence Context remains a conservative placeholder until its own contract exists.
