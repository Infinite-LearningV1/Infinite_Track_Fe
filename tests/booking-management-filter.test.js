...
test("invalid range blocks Apply and active count groups dates", async () => {
  let calls = 0;
  const state = bookingListAlpineData({ getBookings: async () => { calls += 1; return { data: { bookings: [], pagination: {} } }; } });
  ...
  state.appliedQuery.appliedFilters = { status: "pending", dateFrom: "2026-08-11", dateTo: "" };
...
test("booking filter partial and page expose the combined toolbar contract", () => {
  const partial = fs.readFileSync("src/partials/table/booking-table-filter.html", "utf8");
  const page = fs.readFileSync("src/management-booking.html", "utf8");
  for (const token of [":aria-expanded=\"isFilterOpen\"", "aria-controls=\"bookingTableFilterPopover\"", "@click.outside=\"closeFilter()\"", "@keydown.escape.window=\"closeFilter()\"", "x-model=\"draftFilters.status\"", "x-model=\"draftFilters.dateFrom\"", "x-model=\"draftFilters.dateTo\"", "@click=\"applyFilters()\"", "@click=\"clearFilters()\"", "activeFilterCount"]) assert.match(partial, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
