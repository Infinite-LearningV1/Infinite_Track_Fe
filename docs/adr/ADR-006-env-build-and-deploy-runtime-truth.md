...
The default Web FE integration topology is same-origin `/api` through a gateway or proxy. Direct absolute API origins remain an override-only configuration path and must be explicitly injected and verified per environment rather than assumed by service code.
