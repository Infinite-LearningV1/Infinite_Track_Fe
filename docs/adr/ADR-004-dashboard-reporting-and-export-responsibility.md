...
We will keep report/export transport separate from owner-specific dashboard requests. `/summary/reports` remains the selected-period reporting/export path, while `/summary/dashboard-analytics`, `/attendance/geofence-evidence`, `/attendance/today-locations`, and `/analysis/fuzzy-ahp/dashboard?type=discipline|wfa|smart_ac` remain owner-specific dashboard surfaces. Cockpit-specific envelope unwrapping and no-fallback normalization belong in `dashboardCockpitService`, not in the shared request layer.
