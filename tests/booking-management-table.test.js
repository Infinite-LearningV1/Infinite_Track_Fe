...
test("booking table exposes the locked eight-column approval queue", () => {
  ...
  for (const label of ["Pemohon", "Jadwal WFA", "Diajukan", "Alasan", "Lokasi", "Kelayakan", "Status", "Aksi"]) assert.match(table, new RegExp(`>\\s*${label}\\s*<`));
  for (const label of ["ID", "Position", "Notes", "Koordinat"]) assert.doesNotMatch(table, new RegExp(`>\\s*${label}\\s*<`));
  for (const token of ["changeSort", "getSortIcon", "aria-sort", "View Location", "viewLocationDetail", "style=\\\"width"]) assert.doesNotMatch(table, new RegExp(token));
